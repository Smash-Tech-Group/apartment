...
export default function Stays() {
  ...
  const fetchFavorites = async () => {
    try {
      const res = await getFavorites("stay", 1, 100);
  ...
  const handleToggleFavorite = async (booking) => {
    const isFavorited = favoritedIds.includes(booking.id);
    try {
      if (isFavorited) {
        await removeFavoriteByItem("stay", booking.id);
        setFavoritedIds((prev) => prev.filter((id) => id !== booking.id));
      } else {
        await addFavorite({
          item_id: booking.id,
          item_type: "stay",
          item_name: booking.apartment_name,
          item_location: booking.apartment_location,
          item_image: booking.apartment_image,
          item_rating: booking.apartment_rating,
          item_price: booking.total_price,
        });
        setFavoritedIds((prev) => [...prev, booking.id]);
      }
    } catch (err) {
  ...
  const handleConfirmRating = async (ratingValue) => {
    try {
      await submitReview(selectedBooking.id, reviewText, ratingValue, "stay", selectedBooking.apartment_image, selectedBooking.apartment_name);
        ...
        <div className="flex flex-col lg:flex-row gap-6 lg:gap-8">
          ...
          <div className="flex-1">
            <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-2 mb-6 sm:mb-8">
              <h1 className="text-2xl sm:text-3xl lg:text-4xl font-semibold text-gray-900">Stays</h1>
            ...
            {error && (
              <div className="text-center py-16">
                ...
                <button onClick={fetchBookings} className="text-[#FF7D01] font-medium underline">
                  Try again
                </button>
              </div>
            )}
            ...
            {!loading && !error && bookings.length === 0 && (
              <div className="text-center py-16">
                <p className="text-gray-500 text-lg">You have no stays yet.</p>
              </div>
            )}
              ...
              <div className="space-y-6 sm:space-y-10">
                {bookings.map((booking) => (
                  <StayBookingItem
                    ...
                    isFavorited={favoritedIds.includes(booking.id)}
                    onToggleFavorite={() => handleToggleFavorite(booking)}
                  />
                ))}
                  ...
                  <div className="mt-8 text-center">
                    <button
                      onClick={() => {
                        ...
                        setPage(nextPage);
                        getBookings(nextPage).then((res) => {
                          setBookings((prev) => [...prev, ...res.data.items]);
                          setTotalPages(res.data.pages);
                        });
                      }}
      ...
      {selectedBooking && (
        <>
          <ReviewModal
            ...
            bookingTitle={selectedBooking.apartment_name}
...
function StayBookingItem({ booking, onReviewClick, hasReviewed, isFavorited, onToggleFavorite }) {
  ...
  return (
    <div className="mb-6 sm:mb-10">
      <div className="flex flex-col sm:flex-row items-start gap-4">
        ...
        <div className="w-full sm:w-[140px] h-[200px] sm:h-[140px] md:w-[160px] md:h-[160px] rounded-2xl overflow-hidden flex-shrink-0">
          {booking.apartment_image ? (
            <img
              src={getMediaUrl(booking.apartment_image)}
        ...
        <div className="flex-1 flex flex-col w-full">
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 sm:gap-0 mb-3">
            <div className="flex-1">
              <h3 className="text-lg sm:text-xl font-semibold text-gray-900 mb-1">{booking.apartment_name}</h3>
              <p className="text-sm text-gray-500">{booking.apartment_location}</p>
            </div>
            <div className="text-xl sm:text-2xl font-semibold text-gray-900">{formatPrice(booking.total_price)}</div>
          </div>

          {booking.apartment_rating && (
            <div className="mb-4">
              <span className="text-sm font-normal bg-[#FFF1E6] text-[#FF7D01] rounded-full px-3 py-1 inline-block">
                Rating <span className="font-semibold">{booking.apartment_rating}</span>
              </span>
            </div>
          )}
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div className="flex items-center gap-4 sm:gap-6 text-sm overflow-x-auto">
              <div className="flex-shrink-0">
                ...
                <span className="font-normal text-xs text-gray-900">{formatDate(booking.check_in)}</span>
              </div>
              ...
              <div className="flex-shrink-0">
                ...
                <span className="font-normal text-xs text-gray-900">{formatDate(booking.check_out)}</span>
              </div>
              <div className="h-9 w-px bg-[#333333]/50 flex-shrink-0"></div>
              <div className="flex-shrink-0">
      ...
      <div className="flex items-center gap-3 sm:gap-4 mt-4 sm:mt-6">
        ...
        >
          <Heart
            className={`w-5 h-5 sm:w-6 sm:h-6 transition-all duration-300 transform ${
              isFavorited
        ...
        {hasReviewed ? (
          <div className="flex-1 sm:flex-initial px-4 sm:px-5 py-3 sm:py-4 bg-[#FF7D01]/10 text-[#FF7D01] rounded-full font-medium text-sm sm:text-base flex items-center gap-2">
